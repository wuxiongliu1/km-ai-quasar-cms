/**
 * 微信公众号发布 - Node.js 后端模块
 *
 * 此模块由 supabase/functions/wechat-publish/index.ts 迁移而来，
 * 不依赖 Supabase Edge Function，可直接集成到 Express / Koa / Fastify 等 Node.js 后端中。
 *
 * 要求：Node.js >= 18（内置 fetch / FormData / Blob / File / atob）
 */
...
function resolveWechatError(errcode, fallback) {
...
async function uploadImageToWechat(accessToken, imageUrl) {
  let blob
...
async function uploadThumbMedia(accessToken, imageUrl) {
  let blob
...
async function publishArticle(body) {
  ...
  if (!body.appId || !body.appSecret || !body.title || !body.content) {
    throw new Error('缺少必要参数: appId, appSecret, title, content')
  }
  ...
  if (!thumbUrl) {
    throw new Error('缺少封面图，请上传封面或确保文章包含图片')
  }
 ...
 * 使用方式（Express）：
 *
 *   const { wechatPublishHandler } = require('./server/wechat-publish')
 *   app.post('/api/wechat/publish', wechatPublishHandler)
 *
 */
async function wechatPublishHandler(req, res) {
  // 处理 CORS Preflight
  ...
  try {
    ...
    if (body.action === 'getIp') {
      try {
        ...
        res.set({ ...corsHeaders, 'Content-Type': 'application/json' })
        return res.status(500).json({ success: false, message: '获取 IP 失败' })
      }
    }
